...
alter table `GroupsCurators` 
	add constraint `FK_GroupsCurators_CuratorId` foreign key(`CuratorId`) 
								  references `Curators`(`id`),
	add constraint `FK_GroupsCurators_GroupId` foreign key(`GroupId`) 
...
alter table `GroupsStudents` 
	add constraint `FK_GroupsStudents_StudentId` foreign key(`StudentId`) 
								  references `Students`(`id`),
	add constraint `FK_GroupsStudents_GroupId` foreign key(`GroupId`) 
								references `Groups`(`id`);
